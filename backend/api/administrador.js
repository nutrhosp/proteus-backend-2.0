...
module.exports = app => {
    ...
    const remove = async (req, res) => {
        try {
          ...
          const rowsDeleted = await app.db('administrador').del()
    ...
    const post = async (req, res) => {
        let { adm_login, adm_senha, adm_confirmar_senha } = req.body;
        try {
            existsOrError(adm_login, 'Login não informado')
            existsOrError(adm_senha, 'Senha não informada')
            existsOrError(adm_confirmar_senha, 'Confirmação de senha invalida')
            equalsOrError(adm_senha, adm_confirmar_senha, 'Senhas não conferem')

            const admFromDB = await knex('administrador')
                .where({ adm_login: adm_login }).first()
            if (!adm_login){
